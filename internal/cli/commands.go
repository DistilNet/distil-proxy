...
func runInteractiveAuth(cmd *cobra.Command) error {
	...
	return saveAuthKeyWithVerificationKey(cmd, daemonKey, strings.TrimSpace(verifyResp.Email), verificationKey)
}
...
func saveAuthKeyWithVerificationKey(cmd *cobra.Command, key string, email string, verificationKey string) error {
